...
bool MyDB_TableRecIteratorAlt ::advance()
{
    ...
    if (curPage == myTable->lastPage() || curPage == highPage) return false;
...
MyDB_TableRecIteratorAlt ::MyDB_TableRecIteratorAlt(MyDB_TableReaderWriter &myParent,
                                                    MyDB_TablePtr myTableIn, int lowPage,
                                                    int highPageIn)
    : myParent(myParent)
{
    myTable = myTableIn;
