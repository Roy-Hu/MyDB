...
{
    forMe = make_shared<MyDB_Table>(*fromMe->forMe);
...
MyDB_TableReaderWriter ::MyDB_TableReaderWriter(MyDB_TablePtr forMeIn,
                                                MyDB_BufferManagerPtr myBufferIn)
{
    forMe = forMeIn;
    myBuffer = myBufferIn;
...
#define MAX_SIZE 1000
                if (a.first.size() > MAX_SIZE)
                {
                    ...
                    {
                        if (num % a.second == 0) newSet.insert(num);
                    }
                    a.first = newSet;
                }
            }
