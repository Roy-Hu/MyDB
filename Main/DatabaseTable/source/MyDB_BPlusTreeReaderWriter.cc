...
MyDB_RecordIteratorAltPtr MyDB_BPlusTreeReaderWriter ::getSortedRangeIteratorAlt(
    MyDB_AttValPtr low, MyDB_AttValPtr high)
{
    ...
    return make_shared<MyDB_PageListIteratorSelfSortingAlt>(list, lhs, rhs, comparator, myRec,
                                                            lowComparator, highComparator, true);
}

MyDB_RecordIteratorAltPtr MyDB_BPlusTreeReaderWriter ::getRangeIteratorAlt(MyDB_AttValPtr low,
                                                                           MyDB_AttValPtr high)
{
    // this is the list of pages that we need to iterate over
    vector<MyDB_PageReaderWriter> list;
    ...
    // and build the iterator
    return make_shared<MyDB_PageListIteratorSelfSortingAlt>(list, lhs, rhs, comparator, myRec,
                                                            lowComparator, highComparator, false);
}
                                                ...
                                                MyDB_AttValPtr low, MyDB_AttValPtr high)
{
    ...
    else
    {
        ...
        while (temp->advance())
        {
            ...
            if (!comparatorLow()) lowEngaged = true;
            ...
            if (comparatorHigh()) highEngaged = false;
        }
        return false;
    }
...
void MyDB_BPlusTreeReaderWriter ::append(MyDB_RecordPtr appendMe)
{
    ...
    else
    {
        ...
        if (res != nullptr)
        {
            ...
            // add the two records; the first points to the newly-created page, the second to the
            // old root
            newRoot.append(res);
...
MyDB_RecordPtr MyDB_BPlusTreeReaderWriter ::split(MyDB_PageReaderWriter splitMe,
                                                  MyDB_RecordPtr andMe)
{
    ...
    for (void *pos : positions)
    {
        ...
        lhs->fromBinary(pos);
        if (counter < positions.size() / 2) newPage.append(lhs);
        ...
        if (counter > positions.size() / 2) splitMe.append(lhs);
...
MyDB_AttValPtr MyDB_BPlusTreeReaderWriter ::getKey(MyDB_RecordPtr fromMe)
{
    ...
    if (fromMe->getSchema() == nullptr) return fromMe->getAtt(0)->getCopy();
...
function<bool()> MyDB_BPlusTreeReaderWriter ::buildComparator(MyDB_RecordPtr lhs,
                                                              MyDB_RecordPtr rhs)
{
