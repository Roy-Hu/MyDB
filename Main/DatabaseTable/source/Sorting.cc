...
{
    bool skipPred = false;
    if (lhsPred == "bool[true]") skipPred = true;
    ...
    for (int i = 0; i < sortMe.getNumPages(); i++)
    {
        ...
        if (pagesToSort.size() != runSize && i != sortMe.getNumPages() - 1) continue;
        ...
        while (pagesToSort.size() > 1)
        {
            ...
            while (pagesToSort.size() > 0)
            {
                ...
                // merge them
                newPagesToSort.push_back(
                    mergeIntoList(sortMe.getBufferMgr(), getIteratorAlt(runOne),
          ...
          function<bool()> comparator, MyDB_RecordPtr lhs, MyDB_RecordPtr rhs)
{
    ...
    MyDB_RecordIteratorAltPtr myIter =
        buildItertorOverSortedRuns(runSize, sortMe, comparator, lhs, rhs);
