...
public:
    ...
    bool advance() override
    {
        while (true)
        {
            ...
            {
                curPage++;
                if (sortOrNot) forUs[curPage].sortInPlace(comparator, lhs, rhs);
    ...
    MyDB_PageListIteratorSelfSortingAlt(vector<MyDB_PageReaderWriter> &forUsIn,
                                        MyDB_RecordPtr lhsIn, MyDB_RecordPtr rhsIn,
                                        function<bool()> comparatorIn, MyDB_RecordPtr myRecIn,
                                        function<bool()> lowComparatorIn,
