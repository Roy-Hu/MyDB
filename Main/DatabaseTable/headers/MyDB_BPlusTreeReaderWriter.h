...
public:
    ...
    MyDB_BPlusTreeReaderWriter(string nameOfAttToOrderOn, MyDB_TablePtr forMe,
    ...
    // gets an instance of an alternate iterator over the table... this is an
    // iterator that has the alternate getCurrent ()/advance () interface... returned records must
    // be sorted return all records with a key value in the range [low, high], inclusive
...
private:
    ...
    bool discoverPages(int whichPage, vector<MyDB_PageReaderWriter> &list, MyDB_AttValPtr low,
                       MyDB_AttValPtr high);

    // appends a record to the named page; if there is a split, then an MyDB_INRecordPtr is returned
    // that points to the record holding the (key, ptr) pair pointing to the new page.  Note that
    // the new page always holds the lower 1/2 of the records on the page; the upper 1/2 remains in
    // the original page
    MyDB_RecordPtr append(int whichPage, MyDB_RecordPtr appendMe);

    // splits the given page (plus the record andMe) around the median.  A MyDB_INRecordPtr is
    // returned that points to the record holding the (key, ptr) pair pointing to the new page. Note
    // that the new page always holds the lower 1/2 of the records on the page; the upper 1/2
    ...
    // constructs an returns a comparator for the two records given... both must either be IN
    // records for this particular tree, or they must be LN records for this tree, or a combination.
    // The resulting comparator returns true if and only if the first record has a key value less
