...
#define QUNIT_COMPARE(compare, result, expr1, expr2)                                            \
    {                                                                                           \
        ...
        qunit.evaluate(compare, result, s1.str(), s2.str(), #expr1, #expr2, __FILE__, __LINE__, \
                       __FUNCTION__);                                                           \
    };
...
namespace QUnit
{
    ...
    public:
        ...
        void evaluate(bool, bool, std::string, std::string, std::string, std::string, const char *,
                      int, const char *);
    ...
    {
        if (verboseLevel_ > quiet) printStatus();
    }
    ...
    inline void UnitTest::evaluate(bool compare, bool result, std::string val1, std::string val2,
                                   std::string str1, std::string str2, const char *file, int line,
                                   const char *func)
    {
        ...
        if ((ok && !(verboseLevel_ > normal)) || verboseLevel_ == silent) return;
