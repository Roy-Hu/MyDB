...
void Aggregate ::run()
{
    ...
    if (output->getTable()->getSchema()->getAtts().size() !=
        aggsToCompute.size() + groupings.size())
    {
        cout << "error, the output schema needs to have the same number of atts as (# of aggs to "
                "compute + # groups).\n";
        return;
    }
    ...
    for (auto &a : input->getTable()->getSchema()->getAtts()) combinedSchema->appendAtt(a);
    for (auto &a : aggSchema->getAtts()) combinedSchema->appendAtt(a);
    ...
    for (auto &s : aggsToCompute)
    {
        ...
        {
            aggComps.push_back(combinedRec->compileComputation("+ (" + s.second + ", [MyDB_AggAtt" +
                                                               to_string(i) + "])"));
        }
        else if (s.first == MyDB_AggType ::cnt)
        {
            aggComps.push_back(
                combinedRec->compileComputation("+ ( int[1], [MyDB_AggAtt" + to_string(i) + "])"));
        }
        ...
        {
            finalAggComps.push_back(
                combinedRec->compileComputation("[MyDB_AggAtt" + to_string(i++) + "]"));
        }
    }
