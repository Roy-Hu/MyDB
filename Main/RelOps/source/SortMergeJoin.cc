...
void SortMergeJoin ::run()
{
    ...
    function<bool()> leftComp =
        buildRecordComparator(leftInputRec, leftInputRecOther, equalityCheck.first);
    function<bool()> leftCompRev =
        buildRecordComparator(leftInputRecOther, leftInputRec, equalityCheck.first);
    function<bool()> rightComp =
        buildRecordComparator(rightInputRec, rightInputRecOther, equalityCheck.second);

    // now, sort the left and the right
    MyDB_RecordIteratorAltPtr right =
        buildItertorOverSortedRuns(runSize, *rightTable, rightComp, rightInputRec,
                                   rightInputRecOther, rightSelectionPredicate);
    MyDB_RecordIteratorAltPtr left = buildItertorOverSortedRuns(
        runSize, *leftTable, leftComp, leftInputRec, leftInputRecOther, leftSelectionPredicate);

    // and get the schema that results from combining the left and right records
    MyDB_SchemaPtr mySchemaOut = make_shared<MyDB_Schema>();
    for (auto &p : leftTable->getTable()->getSchema()->getAtts()) mySchemaOut->appendAtt(p);
    for (auto &p : rightTable->getTable()->getSchema()->getAtts()) mySchemaOut->appendAtt(p);
    ...
    func leftSmaller = combinedRec->compileComputation(" < (" + equalityCheck.first + ", " +
                                                       equalityCheck.second + ")");
    func rightSmaller = combinedRec->compileComputation(" > (" + equalityCheck.first + ", " +
                                                        equalityCheck.second + ")");
    func areEqual = combinedRec->compileComputation(" == (" + equalityCheck.first + ", " +
                                                    equalityCheck.second + ")");
    ...
    if (!left->advance() || !right->advance()) return;
