...
int main(int numArgs, char **args)
{
    ...
    for (auto &a : allTables)
    {
        ...
        {
            allBPlusReaderWriters[a.first] =
                make_shared<MyDB_BPlusTreeReaderWriter>(a.second->getSortAtt(), a.second, myMgr);
    ...
    while (true)
    {
        ...
        for (string line; getline(cin, line);)
        {
            ...
            if (pos != string ::npos)
            {
                ...
                if (tokens.size() == 1 &&
                    (toLower(tokens[0]) == "exit" || toLower(tokens[0]) == "quit"))
                {
                    cout << "OK, goodbye.\n";
                ...
                if (tokens.size() == 4 && toLower(tokens[0]) == "load" &&
                    toLower(tokens[2]) == "from")
                {
                    ...
                    else
                    {
                        ...
                        // load up the file
                        pair<vector<size_t>, size_t> res =
                            allTableReaderWriters[tokens[1]]->loadFromTextFile(tokens[3]);
                ...
                else
                {
                        ...
                        if (tableName != "nothing")
                        {
                            ...
                            if (allTables[tableName]->getFileType() == "heap")
                            {
                                allTableReaderWriters[tableName] =
                                    make_shared<MyDB_TableReaderWriter>(allTables[tableName],
                                                                        myMgr);
                            }
                            else if (allTables[tableName]->getFileType() == "bplustree")
                            {
                                allBPlusReaderWriters[tableName] =
                                    make_shared<MyDB_BPlusTreeReaderWriter>(
                                        allTables[tableName]->getSortAtt(), allTables[tableName],
                                        myMgr);
                    ...
                    {
                        LogicalOpPtr myPlan =
                            final->buildLogicalQueryPlan(allTables, allTableReaderWriters);
                        if (myPlan != nullptr)
                        {
