...
public:
    ...
    virtual ExprTreePtr getLHS()
    {
        return nullptr;
    }
    virtual ExprTreePtr getRHS()
    {
        return nullptr;
    }
    virtual ExprTreePtr getChild()
    {
        return nullptr;
    }
    virtual bool referencesTable(string alias)
    {
        return false;
    }
    virtual bool referencesAtt(string alias, string attName)
    {
        return false;
    }
    virtual bool hasAgg()
    {
        if (isSum() || isAvg()) return true;
        ExprTreePtr lhs = getLHS();
        ExprTreePtr rhs = getRHS();
        ExprTreePtr child = getChild();
        bool res = false;
        if (lhs != nullptr) res = res || lhs->hasAgg();
        if (rhs != nullptr) res = res || rhs->hasAgg();
        if (child != nullptr) res = res || child->hasAgg();
        return res;
    }
    virtual vector<ExprTreePtr> getAggExprs()
    {
        if (isSum() || isAvg()) return {shared_from_this()};
        if (getChild()) return getChild()->getAggExprs();
        if (getLHS() && getRHS())
        {
            vector<ExprTreePtr> vec1 = getLHS()->getAggExprs();
            vector<ExprTreePtr> vec2 = getRHS()->getAggExprs();

            vec1.insert(vec1.end(), make_move_iterator(vec2.begin()),
    ...
    {
        if (isId()) return {shared_from_this()};
        if (getChild()) return getChild()->getIdentifiers();
        if (getLHS() && getRHS())
        {
