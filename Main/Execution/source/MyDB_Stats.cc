...
double MyDB_Stats ::getAttVals(string forMe)
{
    for (auto a : allAtts)
    {
        if (a.second == forMe) return a.first;
    }
    return 0;
}
...
MyDB_Stats ::MyDB_Stats(MyDB_TablePtr inTable, string aliasName)
{
    ...
    {
        allAtts.push_back(
            make_pair(inTable->getDistinctValues(a.first), aliasName + "_" + a.first));
    }
        ...
        if (a->isEq())
        {
            ...
            if (a->getLHS()->isId() && a->getRHS()->isId())
            {
                ...
                if (lhsAttCnt < 0.00000001)
                    cout << "Bad error!!  I could not find att val " << a->getLHS()->getId()
                         << " during costing.\n";
                ...
                if (lhsAttCnt > 0 && rhsAttCnt > 0)
                {
                    ...
                    if (minAttCnt > rhsAttCnt) minAttCnt = rhsAttCnt;
                    ...
                    for (auto &b : allAtts)
                    {
                        ...
                        {
                            newOne.first = b.first;
                            if (newOne.first > temp->tupleCount) newOne.first = temp->tupleCount;
                        }
                        newOne.second = b.second;
            ...
            {
                if (i != pos) allDisjunctionsNew.push_back(allDisjunctions[i]);
            }
