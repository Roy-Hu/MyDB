...
string joinExprWithoutAlias(vector<ExprTreePtr> expr)
{
    ...
    {
        string res = "&& (" + removeAlias(expr[0]->toString()) + ", " +
                     removeAlias(expr[1]->toString()) + ")";
...
// fill this out!  This should actually run the aggregation via an appropriate RelOp, and then it is
// going to have to unscramble the output attributes and compute exprsToCompute using an execution
// of the RegularSelection operation (why?  Note that the aggregate always outputs all of the
// grouping atts followed by the agg atts. After, a selection is required to compute the final set
// of aggregate expressions)
//
// Note that after the left and right hand sides have been executed, the temporary tables associated
// with the two sides should be deleted (via a kill to killFile () on the buffer manager)
MyDB_TableReaderWriterPtr LogicalAggregate ::execute()
{
    ...
    MyDB_TableReaderWriterPtr tableOut =
        make_shared<MyDB_TableReaderWriter>(outputSpec, tableIn->getBufferMgr());
...
// statistics for the output.  Note that it recursively costs the left and then the right, before
// using the statistics from the left and the right to cost the join itself
...
// Fill this out!  This should recursively execute the left hand side, and then the right hand side,
// and then it should heuristically choose whether to do a scan join or a sort-merge join (if it
// chooses a scan join, it should use a heuristic to choose which input is to be hashed and which is
// to be scanned), and execute the join. Note that after the left and right hand sides have been
// executed, the temporary tables associated with the two sides should be deleted (via a kill to
// killFile () on the buffer manager)
MyDB_TableReaderWriterPtr LogicalJoin ::execute()
{
    ...
    MyDB_TableReaderWriterPtr tableOut =
        make_shared<MyDB_TableReaderWriter>(outputSpec, lTable->getBufferMgr());
    ...
    {
        ScanJoin myOp(lTable, rTable, tableOut, finalSelectionPredicate, projections, hashAtts,
                      "bool[true]", "bool[true]");
        myOp.run();
    }
    else
    {
        ...
        SortMergeJoin myOp(lTable, rTable, tableOut, finalSelectionPredicate, projections,
                           hashAtts[0], "bool[true]", "bool[true]");
        myOp.run();
    }
...
// that a desirable optimization is to somehow set things up so that if a B+-Tree is NOT used, that
// the table scan does not actually do anything, and the selection predicate is handled at the level
// of the parent (by filtering, for example, the data that is input into a join)
