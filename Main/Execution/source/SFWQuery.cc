...
string SFWQuery::createMemoKey(const vector<pair<string, string>> &tables,
                               const vector<ExprTreePtr> &valuesToSelect,
                               const vector<ExprTreePtr> &CNF)
{
    string key;
...
pair<LogicalOpPtr, MyDB_SchemaPtr> SFWQuery ::optimize(vector<pair<string, string>> tables,
    ...
    if (tables.size() == 1)
    {
        ...
        for (const auto &expr : valuesToSelect)
        {
            for (const auto &inputAtt : inputTable->getSchema()->getAtts())
            {
                ...
                {
                    bestSchema->getAtts().push_back(
                        make_pair(aliasName + "_" + inputAtt.first, inputAtt.second));
        ...
        bestOp = make_shared<LogicalTableScan>(
            ...
            make_shared<MyDB_Stats>(inputTable, aliasName), CNF, exprs);
    ...
    else
    {
        ...
        for (const auto &partition : allPartitions)
        {
            ...
            for (const auto &expr : CNF)
            {
                ...
                if (inLeft && inRight)
                    topCNF.push_back(expr);
                else if (inLeft)
                    leftCNF.push_back(expr);
                else
                    rightCNF.push_back(expr);
            }
            ...
            for (int i = 0; i < 2; i++)
            {
                for (auto nameAlias : tables[i])
                {
                    ...
                    for (auto att : table->getSchema()->getAtts())
                    {
                        ...
                        ExprTreePtr idExp = make_shared<Identifier>(
                            strdup(nameAlias.second.c_str()), strdup(att.first.c_str()));

                        if (isSelect || inCNF)
                        {
                            if (i == 0)
                            {
                                leftSchema->getAtts().push_back(
                                    make_pair(nameAlias.second + "_" + att.first, att.second));
                                leftExprs.push_back(idExp);
                            }
                            else if (i == 1)
                            {
                                rightSchema->getAtts().push_back(
                                    make_pair(nameAlias.second + "_" + att.first, att.second));
                        ...
                        if (isSelect)
                        {
                            topSchema->getAtts().emplace_back(nameAlias.second + "_" + att.first,
                                                              att.second);
...
LogicalOpPtr SFWQuery ::buildLogicalQueryPlan(
    ...
    map<string, MyDB_TableReaderWriterPtr> &allTableReaderWriters)
{
    ...
    for (auto iter = tablesToProcess.begin(); iter != tablesToProcess.end();)
    {
        ...
        bool isUsed = any_of(allDisjunctions.begin(), allDisjunctions.end(),
                             [&tableAlias](const ExprTreePtr &expr)
                             { return expr->referencesTable(tableAlias); }) ||
                      any_of(valuesToSelect.begin(), valuesToSelect.end(),
                             [&tableAlias](const ExprTreePtr &expr)
                             { return expr->referencesTable(tableAlias); }) ||
                      any_of(groupingClauses.begin(), groupingClauses.end(),
                             [&tableAlias](const ExprTreePtr &expr)
                             { return expr->referencesTable(tableAlias); });
    ...
    pair<LogicalOpPtr, MyDB_SchemaPtr> best =
        optimize(tablesToProcess, valuesToSelect, allDisjunctions);
    ...
    for (auto a : valuesToSelect)
    {
        schemaFinal->getAtts().push_back(
            make_pair("final_" + to_string(attId++), myRecord.getType(a->toString())));
        ...
        for (auto &expr : valuesToSelect)
        {
            for (const auto &aggExpr : expr->getAggExprs())
            {
                ...
                if (exprNewName.find(aggExprStr) == exprNewName.end())
                {
                    ...
                    aggSchema->getAtts().emplace_back(newName,
                                                      myRecord.getType(aggExpr->toString()));
    ...
    if (areAggs)
    {
        finalOpIn = make_shared<LogicalAggregate>(
            myOp, make_shared<MyDB_Table>("agg", "agg.bin", aggSchema), aggSelections,
            groupingClauses);
    ...
    {
        finalSchema->getAtts().emplace_back("final_" + to_string(attId++),
                                            finalRecord.getType(exprStr));
    }

    return make_shared<LogicalTableScan>(finalOpIn,
                                         make_shared<MyDB_Table>("final", "final.bin", finalSchema),
                                         finalOpIn->cost().second, finalSelections);
}
...
SFWQuery ::SFWQuery(struct ValueList *selectClause, struct FromList *fromClause, struct CNF *cnf,
